...
NS_LOG_COMPONENT_DEFINE ("AtpTest");
  ...
  if (useEcn)
    {
      ...
      if (useAtp)
        {
          Config::SetDefault ("ns3::TcpL4Protocol::SocketBaseType", TypeIdValue(TypeId::LookupByName ("ns3::AtpSocket")));
          Config::SetDefault ("ns3::AtpSocket::AtpWeight", DoubleValue (1.0 / 16));
        }
    }
}
...
main (int argc, char *argv[])
{
  ...
  // LogComponentEnable ("AtpSocket", LOG_LEVEL_DEBUG);
  bool useEcn = 1;              // 使用ECN
  bool useAtp = 1;              // 使用ATP
  std::string pathOut;          // 输出路径
  bool writeForPlot = 1;        // 输出plot
  bool writePcap = 1;           // 输出pcap
  bool flowMonitor = 1;         // 流监控
  bool writeThroughput = 1;     // 直写

  bool printRedStats = true;    // 输出Red状态
  ...
  pathOut = "./atp_res"; // Current directory
  CommandLine cmd;
  cmd.AddValue ("useEcn", "<0/1> to use ecn in test", useEcn);
  cmd.AddValue ("useAtp", "<0/1> to use atp in test", useAtp);
